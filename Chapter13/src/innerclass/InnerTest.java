...
class OutClass { // 외부 클래스
	private int num = 10;			// 외부클래스 멤버변수 = 인스턴스변수
	private static int sNum = 20;	// 외부클래스 정적변수 = static변수
	
	InClass inClass;	// 멤버변수 == 인스턴스변수
	
	public OutClass() {
		inClass = new InClass();	// 단 외부 클래스 생성된 후 내부 클래스 생성가능
	}
	...
	static class InStaticClass {	// 정적 내부 클래스
		...
		void inTest()	{	// 정적 내부 클래스의 일반메서드
			// System.out.println("OutClass num = " + num + "(외부클래스 인스턴수 변수)"); 외부클래스 인스턴스변수 사용불가
...
//			inNum += 10;	// 외부 클래스와 정적내부 클래스의 인스턴스변수는 사용할수 없음
	...
	class InClass{	// 인스턴스 내부 클래스
		...
		static int sInNum = 200;	// 인스턴스 내부클래스에서 정적변수 선언불가는 jdk8
		
		void inTest()	{
			System.out.println("OutClass num = " + num + "(외부클래스 인스턴수 변수)");
			System.out.println("OutClass sNum = " + sNum + "(외부클래스의 정적 변수)");
		}
	}
